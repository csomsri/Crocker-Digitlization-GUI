...
class MetricsTest(unittest.TestCase):
    ...
    def test_oscillation_penalized_including_offset(self):
        rows = [(i*.05, 10-(2+math.sin(i*.05*math.tau*2)), 2+math.sin(i*.05*math.tau*2), 0) for i in range(100)]
        m = evaluate_trial(rows, 10)
        ...
        flat = evaluate_trial([(i*.05,8,2,0) for i in range(100)],10)
    ...
    def test_overshoot_is_diagnostic_only(self):
        ...
        m=evaluate_trial([(i*.1,10,0,0) for i in range(30)],10)
...
class PythonBOTest(unittest.TestCase):
    def test_same_ui_seven_python_trials(self):
        ...
        try:
            ...
            self.assertEqual(dialog.findChild(QTableWidget,'pidResponseMetrics').columnCount(),11)
            dialog.close()
            other.stop_backend()
            ...
            p.tuning_samples=[(i*.05, 10-(2+math.sin(i*.05*math.tau*2)),
                               2+math.sin(i*.05*math.tau*2), 0) for i in range(100)]
